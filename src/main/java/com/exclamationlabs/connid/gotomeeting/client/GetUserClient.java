...
public class GetUserClient {
    ...
    public GetUserClient(GotoMeetingConnection input) {
        connection = input;
    }

    public User execute(String userKey) {
        HttpGet request = connection.createGetRequest("/users/" + userKey);
        User response = connection.executeRequest(request, User.class);
